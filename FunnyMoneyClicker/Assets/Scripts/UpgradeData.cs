...
public class UpgradeData
{
    ...
    [HideInInspector] public float costIncreaseRate; // Multiplier for cost growth each level (e.g. cost *= 1.15)
    [HideInInspector] public float productionIncreaseRate; // Multiplier for production increase each level (e.g. production *= 1.05)
    [HideInInspector] public float baseInterval; // Time in seconds between each automatic production tick
    ...
    public UpgradeData()
    {
        costIncreaseRate = 1.145f;
        productionIncreaseRate = 1.07f;
        baseInterval = 1f;
    }
}
