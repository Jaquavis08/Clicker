...
public class UpgradeData
{
    ...
    public float costIncreaseRate = 1.15f; // Multiplier for cost growth each level (e.g. cost *= 1.15)
    public float productionIncreaseRate = 1.05f; // Multiplier for production increase each level (e.g. production *= 1.05)
    public float baseInterval = 1f; // Time in seconds between each automatic production tick
    ...
    [HideInInspector]
    public float currentTime; // Internal timer used to track production intervals (hidden in inspector)
}
